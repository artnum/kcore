...
KQueryExpr.fromString = function (string, options = {}) {
    ...
    for (let value of values) {
        ...
        if (value.startsWith('"') || value === '"' || value.endsWith('"')) {
            ...
            if (state.inexact) {
                if (buffer === '') { pushToExpression(value, state.phone) }
                else { pushToExpression(`${buffer} ${value}`, state.phone) }
                buffer = ''
                state.inexact = false
                continue
            }
        ...
        pushToExpression(`*${value}*`, state.phone)
    }

    if (state.inexact) {
        pushToExpression(buffer, state.phone)
    }
